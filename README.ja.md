...
> **TRTC と Tencent Cloud の関係は？** TRTC の音声エージェントサービスは Tencent Cloud 上で動作しています。簡単に言うと、TRTC は顧客と AI エージェント間の音声通話を受け持ち、Tencent Cloud はバックエンド（権限管理、サービス設定、課金など）を担当します。両者は同じログインシステムを共有しているため、一度の登録で両方使えます。
