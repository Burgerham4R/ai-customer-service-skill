...
[Tencent RTC](https://trtc.io)（リアルタイムコミュニケーション）は、世界中の数千の企業にリアルタイム音声・動画・会話型 AI 体験を提供しています。200 以上の国と地域をカバーするグローバルエッジネットワークを通じて、300ms 未満の超低遅延を大規模に実現します。
...
> **なぜ Tencent Cloud API キーが必要なのか？** この Skill は TRTC ブランドで展開されていますが、セットアップ中に Tencent Cloud API キーの入力も求められます。それぞれの役割は次のとおりです：
> - **TRTC (trtc.io)** はリアルタイムの音声・メディアチャネルを担当 —— 顧客と AI エージェント間の実際の会話
> - **Tencent Cloud** はバックグラウンドでコントロールプレーンを提供 —— 一時アクセストークンの発行、権限管理、サービスオーケストレーション
> - TRTC アカウントと Tencent Cloud アカウントは**統一ログインシステム**で連携 —— 一度の登録で、ログインセッションが自動同期されます
