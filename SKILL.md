...
> 演示路径（A）用 `SKILL_ROOT`（取能力源 + 启动 core）和 `PROJECT_ROOT`（Demo 产物落地）；
> 集成路径（B）同时用到 `SKILL_ROOT`（取能力源 + 启动 core）和 `PROJECT_ROOT`（集成目标）。
> 二者**可能相同也可能不同**，不要混用。
...
| 参数 | 默认 | 说明 |
|---|---|---|
| 部署目录 | `$PROJECT_ROOT/ai-customer-service-demo/` | 演示 UI 落地位置（独立于 Skill 文件夹，方便用户后续调整）；如用户要求改，用其指定目录 |
...
- 产物部署到 `$PROJECT_ROOT/ai-customer-service-demo/`（独立于 Skill 目录，方便用户后续修改）
- 工具（一条命令创建目录并拷贝）：
  ```bash
  execute_command(
  ...
  ```
- 期望：`$PROJECT_ROOT/ai-customer-service-demo/` 下出现 `index.html / app.js / styles.css / data.js / mock-shop.json / tokens.css` + `admin/` 子目录，且 `.env` 中已写入 `WEB_DEMO_DIR`
...
| `cp "$SKILL_ROOT"/scenarios/customer-service/ui/voice-customer-service/{index.html,app.js,styles.css,data.js,mock-shop.json,tokens.css} "$PROJECT_ROOT"/ai-customer-service-demo/` | UI overlay（仅路径 A）|
| `cp -R "$SKILL_ROOT"/scenarios/customer-service/ui/admin-board/. "$PROJECT_ROOT"/ai-customer-service-demo/admin/` | 工单看板挂载（仅路径 A）|
| `mkdir -p "$PROJECT_ROOT"/ai-customer-service-demo/admin` | 创建 Demo 部署目录（仅路径 A）|
| `echo "WEB_DEMO_DIR=<path>" >> "$SKILL_ROOT"/capabilities/conversation-core/.env` | 写入 Demo 目录路径（仅路径 A）|
